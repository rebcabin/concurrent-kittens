...
=(+ 1 (* 2 3))= sailing safely on the waves.
...
$\kitTwoSuffixx$, we don't yet know /to what value/ $y$ gets
...
names. That's /alpha renaming/. It might harmlessly un-shadow
some names.
...
before =say= or =say= before =hear= --- =par= is the captain
and doesn't care; =par= is commutative.
Also, because any number of napping kittens in =par= boats is
