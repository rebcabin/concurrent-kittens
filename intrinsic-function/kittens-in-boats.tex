...
\texttt{(\& z (| x y))} sailing safely on the waves. Venus knows you
didn't mean \texttt{(| x (\& y z))}.

Have you been burnt by C code like \texttt{x|y\&z} or \texttt{x/y*z}?
Mathematicians  normally read \(\,x/yz\,\) as
\(\,x/(yz)\,\),\footnote{I haven't found a recent official source
to cite from the American Mathematical Society and the
American Physical Society, but there is this
\url{https://math.stackexchange.com/questions/213406/does-x-yz-mean-x-yz-or-x-yz}}
but I know of no programming language that treats \texttt{x/y*z} like
that. Rather, programming languages treat it like \((x z)/y\).
...
and the pi calculus\footnote{\url{https://en.wikipedia.org/wiki/\%CE\%A0-calculus}\label{orgb058f63}}
...
lcompilers\footnote{\url{https://github.com/lcompilers}\label{orgce371e5}} [\textit{sic}]
...
for lfortran and lpython.\textsuperscript{\ref{orgce371e5}}

\subsection{``How did you write this?''}
\label{sec:org3854b23}
...
\label{sec:orgeaf6f56}

From the wiki page:\textsuperscript{\ref{orgb058f63}}
